...
plan:
	sh -c "source ~/.secrets/do.sh ; exec $(TF_PATH) plan -state=$(TF_STATE) -refresh=true $(args) $(TF_DIR)"

plan.gen:
	sh -c "source ~/.secrets/do.sh ; exec $(TF_PATH) plan -state=$(TF_STATE) -refresh=true -out $(TF_PLAN) $(args) $(TF_DIR)"

plan.apply:
	sh -c "source ~/.secrets/do.sh ; exec $(TF_PATH) apply -state=$(TF_STATE) $(args) $(TF_PLAN)"
